...
router.post('/list-product',jsonParser,async (req,res)=>{
    ...
    if(categoryDetail){
        if(categoryDetail.parent){
            ...
            const subCat = await category.find({_id:ObjectID(categoryDetail._id)})
        ...
        const productList = await ProductSchema.aggregate([
            ...
            const categoryList = data.category?await category.find({parent:categoryDetail._id.toString()}):
